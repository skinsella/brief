...
def main() -> int:
    ...
    print("\n[7/8] Computing AIOE academic benchmark…")
    aioe_xlsx = REPO / "raw" / "aioe_appendix.xlsx"
    if aioe_xlsx.exists():
        if not run("AIOE benchmark",
                   ["uv", "run", "python", str(SCRIPTS / "07_aioe_benchmark.py")]):
            failures.append("07_aioe_benchmark")
    else:
        print(f"  ↺ skipped ({aioe_xlsx.name} missing; download from "
              f"github.com/AIOE-Data/AIOE to enable)")

    print("\n[8/8] Building site/data.json…")

...
LEFT_COL_MAX_X = 140  # observed: left-column words sit at x0 ≈ 72–130; right at ≈ 155+


def _words_by_line(words: list, *, x_min: float = -1e9, x_max: float = 1e9) -> list[dict]:
    """Cluster words at similar y-positions into a single line, return
    list of {top, lines: [str]} in reading order. Filters by x band."""
    flt = sorted(
        (w for w in words if x_min <= w["x0"] <= x_max),
    ...
    cur_words: list[str] = []
    for w in flt:
        if cur_y is None or abs(w["top"] - cur_y) > 3:
            if cur_words:
                out.append({"top": cur_y, "text": " ".join(cur_words).strip()})
            cur_y = w["top"]
            cur_words = [w["text"]]
        else:
...
def _split_blocks_at_shortage_anchors(left_lines: list[dict]) -> list[dict]:
    """The left column repeats the pattern:
        <group name (1–3 lines)>
        <blank>
        Skills shortage:
        <shortage value (1–4 lines)>
        <blank>
    Group successive lines into one block per group. Returns list of
    {top_start, top_end, group_name, shortage}.
    """
    blocks: list[dict] = []
    i = 0
    while i < len(left_lines):
        # Find next "Skills shortage:" or "Shortage:" line.
        sh_idx = None
        for j in range(i, len(left_lines)):
            t = left_lines[j]["text"]
            if t.lower() in ("skills shortage:", "shortage:"):
                sh_idx = j
                break
        if sh_idx is None:
            break
        # The group name is the lines immediately before sh_idx — contiguous
        # in the left column, stopping at a large vertical gap (which means
        # we've crossed into the previous group's shortage value or page
        # header). Group names are usually ≤4 short lines.
        name_lines: list[str] = []
        k = sh_idx - 1
        prev_y = left_lines[sh_idx]["top"]
        while k >= i:
            txt = left_lines[k]["text"]
            ty = left_lines[k]["top"]
            if not txt:
                break
            if prev_y - ty > 25:
                break  # gap → previous block
            name_lines.insert(0, txt)
            prev_y = ty
            if len(name_lines) >= 4:
                break
            k -= 1
        # Shortage value: lines after sh_idx up to next group name or
        # end of column. The shortage value is typically a short noun
        # phrase across 1–5 lines; we stop on (a) the next anchor,
        # (b) a footnote line (starts with a 1- or 2-digit number and a
        # URL or descriptive text), (c) the page-footer "National Skills",
        # or (d) a vertical gap > 25pt suggesting next group's heading.
        value_lines: list[str] = []
        prev_y = left_lines[sh_idx]["top"]
        for n in range(sh_idx + 1, len(left_lines)):
            tt = left_lines[n]["text"]
            ty = left_lines[n]["top"]
            if tt.lower() in ("skills shortage:", "shortage:"):
                break
            if re.match(r"^\d{1,3}\s+https?://", tt):  # footnote URL
                break
            if tt.startswith("National Skills"):
                break
            if ty - prev_y > 25:
                # Gap → next group's name starts here, stop.
                break
            if tt:
                value_lines.append(tt)
            prev_y = ty
        # Find next anchor purely for block-iteration purposes (i in main loop).
        next_sh = None
        for n in range(sh_idx + 1, len(left_lines)):
            if left_lines[n]["text"].lower() in ("skills shortage:", "shortage:"):
                next_sh = n
                break

        blocks.append({
            "top_start": left_lines[k]["top"] if k >= 0 and k < len(left_lines) else left_lines[sh_idx]["top"],
            "top_end": left_lines[next_sh - 1]["top"] if next_sh else (left_lines[-1]["top"] + 50),
            "group_name": " ".join(name_lines).strip(),
            "shortage": " ".join(value_lines).strip(),
        })
        i = next_sh if next_sh is not None else len(left_lines)
    return blocks


def extract_two_column_narratives_by_name(
    pdf: "pdfplumber.PDF", section: dict, candidate_names: list[str]
) -> dict[str, dict]:
    """For every known candidate group name, scan the section's pages for
    a left-column block whose joined text matches it (token-overlap),
    then collect the right-column text whose y-position spans the same
    vertical range up to the next match.

    Robust to multi-page narratives, missing shortage anchors, and
    abbreviations in the left column (e.g. "ICT profs. n.e.c." matched
    to "ICT professionals n.e.c.").
    """
    STOP = {"and", "the", "of", "in", "for", "etc", "n", "e", "c"}
    def toks(s: str) -> set[str]:
        return {t for t in re.findall(r"\w+", s.lower()) if t not in STOP}

    name_tokens = {n: toks(n) for n in candidate_names}

    out: dict[str, dict] = {}
    matches: list[tuple[int, float, str]] = []  # (page_idx, top, name)
    left_lines_by_page: dict[int, list[dict]] = {}
    right_lines_by_page: dict[int, list[dict]] = {}

    end = section["end"] if section["end"] is not None else len(pdf.pages)
    # Skip the first page of each section — that's the figure + indicator
    # table page, where group names appear in the indicator table but not
    # in narrative form. Matching there would capture table headers as
    # narrative text.
    for pg_idx in range(section["start"] + 1, end):
        page = pdf.pages[pg_idx]
        words = page.extract_words(use_text_flow=False)
        if not words:
            continue
        right_lines = _words_by_line(words, x_min=LEFT_COL_MAX_X, x_max=1e9)
        left_lines = _words_by_line(words, x_min=60, x_max=LEFT_COL_MAX_X)
        left_lines_by_page[pg_idx] = left_lines
        right_lines_by_page[pg_idx] = right_lines

        # Slide a 1-4 line window over left_lines, joining them and
        # checking token overlap against each candidate name.
        for i in range(len(left_lines)):
            for window_len in (1, 2, 3, 4):
                if i + window_len > len(left_lines):
                    break
                joined = " ".join(l["text"] for l in left_lines[i:i + window_len])
                cand_tokens = toks(joined)
                if not cand_tokens:
                    continue
                best, best_score = None, 0
                for name, ntok in name_tokens.items():
                    score = len(cand_tokens & ntok)
                    # Require a substantial overlap relative to candidate's
                    # token count (≥60% of the candidate tokens AND ≥60%
                    # of the name's tokens, to avoid spurious matches).
                    if score == 0:
                        continue
                    if score / max(len(ntok), 1) >= 0.6 and score / len(cand_tokens) >= 0.6:
                        if score > best_score:
                            best_score = score
                            best = name
                if best:
                    matches.append((pg_idx, left_lines[i]["top"], best))
                    break

    # Sort matches in document order, then for each match collect the
    # right-column text between this match's y and the next match's y
    # (across pages).
    matches.sort(key=lambda m: (m[0], m[1]))
    for idx, (pg, y, name) in enumerate(matches):
        # Determine vertical range: this y on this page, up to the next
        # match's (pg, y), or end of this page if no next or next is later.
        nxt_pg, nxt_y = (matches[idx + 1][0], matches[idx + 1][1]) if idx + 1 < len(matches) else (None, None)

        pieces: list[str] = []
        # Collect from this page from y onwards.
        for rl in right_lines_by_page.get(pg, []):
            if rl["top"] >= y - 4:
                if nxt_pg == pg and rl["top"] >= nxt_y - 4:
                    break
                pieces.append(rl["text"])
        # Spill onto subsequent pages until the next match's page.
        if nxt_pg is not None and nxt_pg > pg:
            for spill_pg in range(pg + 1, nxt_pg + 1):
                for rl in right_lines_by_page.get(spill_pg, []):
                    if spill_pg == nxt_pg and rl["top"] >= nxt_y - 4:
                        break
                    pieces.append(rl["text"])

        narrative = " ".join(pieces).strip()
        # Repair PDF kerning artefacts: "o peratives" → "operatives",
        # "asignificant" → "a significant", etc.
        narrative = re.sub(r"\b([a-z])\s+([a-z]{2,})\b", r"\1\2",
                           narrative, flags=re.IGNORECASE)
        # Common "a<word>" stuck-together artefacts.
        narrative = re.sub(r"\b(a)([A-Z][a-z]+)", r"\1 \2", narrative)
        # Strip page-footer remnants that leak into the right column when
        # narratives span pages. Patterns observed:
        #   "Bulletin October 2025 167"
        #   "National Skills Bulletin October 2025 167"
        #   "2025 - Securing Ireland's Future Q2 2025 | Central Bank ..."
        #   "<NN> https://..."
        narrative = re.sub(
            r"\s+(?:National Skills\s+)?Bulletin\s+October\s+2025\s+\d+", " ",
            narrative, flags=re.IGNORECASE,
        )
        narrative = re.sub(
            r"\s*\d{4}\s*-\s*Securing\s+Ireland.*?Bulletin\s+October\s+2025\s+\d+",
            " ", narrative, flags=re.IGNORECASE,
        )
        narrative = re.sub(r"\s+\d{1,3}\s+https?://\S+.*$", "", narrative)
        narrative = re.sub(r"\s+", " ", narrative).strip()

        # Locate shortage tag for this match: scan the left column on the
        # same page after the name's y for "Skills shortage:" within ~150pt.
        shortage = ""
        for ll_pg in (pg, pg + 1 if pg + 1 < end else None):
            if ll_pg is None:
                break
            for j, ll in enumerate(left_lines_by_page.get(ll_pg, [])):
                if ll["top"] < y - 2 and ll_pg == pg:
                    continue
                if ll["text"].lower() in ("skills shortage:", "shortage:"):
                    # Collect lines after this until ~25pt gap or page end.
                    vlines: list[str] = []
                    prev_y = ll["top"]
                    for k in range(j + 1, len(left_lines_by_page[ll_pg])):
                        nxt = left_lines_by_page[ll_pg][k]
                        if nxt["top"] - prev_y > 25:
                            break
                        if re.match(r"^\d{1,3}\s+https?://", nxt["text"]):
                            break
                        if nxt["text"].startswith("National Skills"):
                            break
                        vlines.append(nxt["text"])
                        prev_y = nxt["top"]
                    shortage = " ".join(vlines).strip()
                    break
            if shortage:
                break

        if name not in out:
            out[name] = {"narrative": narrative, "shortage": shortage}
        else:
            # Dedupe: if we matched the same name twice (e.g. spans two pages
            # where the name reappears), keep the longer narrative.
            if len(narrative) > len(out[name]["narrative"]):
                out[name]["narrative"] = narrative
            if not out[name]["shortage"]:
                out[name]["shortage"] = shortage

    return out


def extract_two_column_narratives(pdf: "pdfplumber.PDF", section: dict) -> dict[str, dict]:
    """Walk every page in this section, do x-coordinate column splitting,
    return {group_name → {narrative, shortage}}.

    Pages in Section 10 have:
      • A figure / sector-overview page first (skipped — full-width text).
      • One or more "Overall Outlook" pages (full-width text, also skipped).
      • Per-group narrative pages with a 2-column layout: group name +
        shortage tag in the left column, narrative paragraph in the right.

    We identify per-group blocks in the left column (anchored on "Skills
    shortage:"), then take the right-column text whose y-position falls
    inside each block's vertical extent as the narrative.
    """
    out: dict[str, dict] = {}
    end = section["end"] if section["end"] is not None else len(pdf.pages)
    for i in range(section["start"], end):
        ...
        if not words:
            continue
        right_lines = _words_by_line(words, x_min=LEFT_COL_MAX_X, x_max=1e9)
        left_lines_raw = _words_by_line(words, x_min=60, x_max=LEFT_COL_MAX_X)

        # The "Overall Outlook" intro on the first page of each section is
        # full-width text that *starts* at x=72, so it leaks into the
        # left-column band when filtered by x alone. Distinguish:
        #   * 2-column rows  → short left text (≤4 words) AND a right-column
        #     line within ±12pt of the same y exists.
        #   * Full-width rows → drop them.
        # We always keep the "Skills shortage:" anchor itself regardless.
        right_y_arr = sorted(l["top"] for l in right_lines)

        def has_right_neighbour(y: float, tol: float = 12.0) -> bool:
            for ry in right_y_arr:
                if abs(ry - y) <= tol:
                    return True
                if ry > y + tol:
                    return False
        ...
        left_lines = [
            l for l in left_lines_raw
            if (
                l["text"].lower() in ("skills shortage:", "shortage:")
                or (len(l["text"].split()) <= 4 and has_right_neighbour(l["top"]))
            )
        ]
        anchors = [l for l in left_lines
                   if l["text"].lower() in ("skills shortage:", "shortage:")]
        if not anchors:
            continue

        blocks = _split_blocks_at_shortage_anchors(left_lines)
        for blk in blocks:
            if not blk["group_name"]:
                continue
            # Collect right-column lines whose top falls inside this block's
            # vertical extent.
            narrative_lines = [
                l["text"] for l in right_lines
                if blk["top_start"] - 4 <= l["top"] <= blk["top_end"] + 4
            ]
            narrative = " ".join(narrative_lines).strip()
            # Repair some common PDF artefacts ("o p eratives" → "operatives").
            narrative = re.sub(r"\b([a-z])\s+([a-z]{2,})\b", r"\1\2", narrative,
                               flags=re.IGNORECASE)
            shortage = re.sub(r"\s+", " ", blk["shortage"]).strip()
            name = re.sub(r"\s+", " ", blk["group_name"]).strip()
            # Dedup if same group is split across pages.
            if name in out:
                out[name]["narrative"] += " " + narrative
                if not out[name]["shortage"] and shortage:
                    out[name]["shortage"] = shortage
            else:
                out[name] = {"narrative": narrative, "shortage": shortage}
    return out
...
def main() -> None:
    ...
    with pdfplumber.open(PDF) as pdf:
        ...
        # Per-section narrative extraction (2-column PDF parsing).
        # Pass the list of candidate titles for each section, get back a
        # {title → {narrative, shortage}} dict.
        title_to_narrative: dict[str, dict] = {}
        for sect_info in section_index_by_name.values():
            sid = id(sect_info)
            sect_entries = entries_by_section.get(sid, [])
            if not sect_entries:
                continue
            cand_names = [e["title"] for e in sect_entries]
            section = sect_info["section"]
            found = extract_two_column_narratives_by_name(pdf, section, cand_names)
            title_to_narrative.update(found)

        n_with_narrative = sum(
            1 for v in title_to_narrative.values() if v.get("narrative")
        )
        print(f"Narratives recovered: {n_with_narrative} / {len(entries)} groups")
        ...
        for entry in entries:
            ...
            narrative = title_to_narrative.get(
